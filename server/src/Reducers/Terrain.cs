...
public partial class Terrain
{
    ...
    [Reducer]
    public static void DeleteTerrainTiles(ReducerContext ctx, int x, int y, float radius)
    {
        var tilesToDelete = ctx.Db.Terrain.Iter()
            .Where(t => Vector2.Distance(new Vector2(t.Position.X, t.Position.Y),
        ...
        foreach (var tile in tilesToDelete)
        {
            ctx.Db.Terrain.Id.Delete(tile.Id);
            Log.Info(
                $"[DeleteTerrainTiles] Deleted terrain tile at ({tile.Position.X}, {tile.Position.Y}) with id {tile.Id}.");
        }

        Log.Debug($"[DeleteTerrainTiles] Deleted {tilesToDelete.Count} tiles around ({x}, {y}) with radius {radius}.");
    }
    ...
    private static void GenerateTiles(ReducerContext ctx, int seed)
    {
        ...
        var terrainDict = new Dictionary<DbVector2, pillz.server.Tables.Terrain>();
        ...
        for (var x = 0; x < width; x++)
        {
            for (var y = 0; y < height; y++)
            {
                if (terrain[x, y])
                {
                    var pos = new DbVector2(x - halfWidth, y - halfHeight);
                    var tile = new pillz.server.Tables.Terrain { Position = pos, IsSpawnable = false };
                    var inserted = ctx.Db.Terrain.Insert(tile);
                    terrainDict[pos] = inserted;
                }
            }
        }
        ...
        GenerateSpawnLocations(ctx, width, height, terrain, halfWidth, halfHeight, terrainDict);
        GeneratePortalLocations(ctx, width, height, terrain, halfWidth, halfHeight);
    }

    private static void GenerateSpawnLocations(ReducerContext ctx, int width, int height, bool[,] terrain,
        int halfWidth, int halfHeight, Dictionary<DbVector2, pillz.server.Tables.Terrain> terrainDict)
    {
        for (var x = 2; x < width - 2; x++)
        {
            for (var y = 2; y < height - 3; y++)
            {
                ...
                // Convert to world position of the GROUND tile (below empty space)
                var gx = x - halfWidth;
                var gy = (y - 1) - halfHeight; // <- y - 1 is the terrain tile below
                var groundPos = new DbVector2(gx, gy);

                if (terrainDict.TryGetValue(groundPos, out var terrainRow))
                {
                    terrainRow.IsSpawnable = true;
                    ctx.Db.Terrain.Id.Update(terrainRow);
                }
            }
        }
    }
